...
<div class="col-lg-12">
<?=form_open_multipart('secure/retailer/sell', array('class'=>'form-horizontal'));?>
							<!-- Traffic sources -->
							<div class="panel panel-flat">
								<div class="panel-heading">
								...
								<div class="container-fluid">
                                    ...
                                    <div class="col-lg-4">
                                    <div class="form-group">
												<label class="col-lg-4 control-label">Retailer List:</label>
												<div class="col-lg-8">
													<select class="form-control select2" name="retailer_id" id="retailer_id" required>
                                                    <option value="">Select</option>
                                                    <?php  foreach($retailerList as $p){?>
                                                    <option value="<?=$p['id'];?>"><?=$p['retailer_name'];?></option>
                                                    <?php }?>
                                                    </select>
                                                    <span class="text-danger"><?=form_error('retailer_id');?></span>
												</div>
											</div>
                                    ...
                                    <div class="form-group">
                                            ...
                                            <div class="col-lg-8">
                                            <input type="text" class="form-control" name="bill_no" id="bill_no" value=""/>
                                    ...
                                    <div class="row">    
                                        ...
                                        <th style="width:15% !important;">Retailer Amount</th>
 ...
 if(product_id ==''){
	...
	if (product_id !=''){
		var bill_date = $('#bill_date').val();
				...
				$.ajax({
                type:'POST',
                url:'<?=base_url('secure/retailer/getproductdetails');?>',
				...
				html += '<td><input type="text" name="rowdata['+newsrno+'][purchage_price]" id="product_price_'+newsrno+'" class="form-control" value="'+data.purchage_price+'"  onChange="changeproductamount('+newsrno+')"/></td>';
				 html += '<td><input type="text" name="rowdata['+newsrno+'][retailer_price]" id="retailer_price_'+newsrno+'" class="form-control" value="'+data.retailer_price+'"  onChange="changeproductamount('+newsrno+')"/></td>';
...
function addproductpart(){
		...
		if(product_part_id !=''){
				...
				$.ajax({
                type:'POST',
                url:'<?=base_url('secure/retailer/getproductpartdetail');?>',
                ...
                success:function(data){
				  ...
				  html += '<td><input type="hidden" name="rowdata['+newsrno+'][product_part_id]" value="'+data.product_id+'"/>'+data.part_name+'</td>';
				...
				html += '<td><input type="text" name="rowdata['+newsrno+'][purchage_price]" id="product_price_'+newsrno+'" class="form-control" value="'+data.purchage_price+'"  onChange="changeproductpartamount('+newsrno+')"/></td>';
				 html += '<td><input type="text" name="rowdata['+newsrno+'][retailer_price]" id="retailer_price_'+newsrno+'" class="form-control" value="'+data.retailer_price+'"  onChange="changeproductpartamount('+newsrno+')"/></td>';
				 html += '<td><input type="text" name="rowdata['+newsrno+'][total_purchase_price]" id="total_purchase_price_'+newsrno+'" class="form-control" value="0" readonly/><input type="hidden" name="total_purchase_price[]" id="hidden_total_purchase_price_'+newsrno+'" value="0"/></td>';
				  html += '<td><span class="btn btn-info" id="removeRow"><i class="fa fa-close"</span></td>';
				  html += '</tr>';
...
function changeproductamount(rowid){
		...
		var retailer_price = parseFloat($('#retailer_price_'+rowid).val());
		var purchage_price = 0;
		
		console.log(product_id+','+stockunit+','+price+','+retailer_price);
		if( stockunit !=0 && price !=0 && retailer_price !=0){
		$.ajax({
                type:'POST',
                url:'<?=base_url('secure/retailer/changeproductamount');?>',
                data:'product_id='+product_id+'&price='+price+'&retailer_price='+retailer_price,
				dataType:'JSON',
                success:function(data){
					console.log(data);
					if(data.status == true){
						var totalPrice = parseFloat(stockunit * retailer_price);
...
function changeproductpartamount(rowid){
		...
		$.ajax({
                type:'POST',
                url:'<?=base_url('secure/retailer/changeproductpartamount');?>',
                data:'product_id='+product_id+'&price='+price+'&retailer_price='+retailer_price,
				dataType:'JSON',
                success:function(data){
					console.log(data);
