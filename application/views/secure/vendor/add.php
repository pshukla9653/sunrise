...
<?=form_open_multipart('secure/vendor/add', array('class'=>'form-horizontal','id'=>'vendorform')); ?>
								...
								<div class="row">
									<div class="col-md-6">
										<fieldset>
                                            ...
                                            <input type="hidden" class="form-control" placeholder="Vendor Id"  id="hideid" name="hideid" >
												<label class="col-lg-3 control-label">Name of the Vendor:</label>
												<div class="col-lg-9">
													<input type="text" class="form-control" placeholder="Name of the Vendor" name="vendor_name" id="vendor_name" onkeyup="ajaxSearch()" value="<?=set_value('vendor_name');?>">
                                                    <span class="text-danger"><?=form_error('vendor_name');?></span>
                                                    <div id="suggestions">
...
<script>
	...
	function getDetail(getId)
		{	
				...
				$('#hideid').val(getId);
		   		if(getId){
						$.ajax({
							type:'POST',
							url:'<?php echo base_url('secure/vendor/getVendorAllData');?>',
							data:'getId='+getId,
							dataType: 'json',
		...
		function ajaxSearch()
        {
                var input_data = $('#vendor_name').val();
                ...
                else
                {
                    ...
                    var post_data = {
                        'vendor_name': input_data
                    };

                    $.ajax({
                        type: "POST",
                        url: '<?php echo base_url('secure/vendor/getVendorSearch');?>',
                        data: post_data,
                        success: function (data) {
                            // return success
                            if (data.length > 0) {
