...
<?php echo form_open('secure/group/add/',array("class"=>"form-horizontal")); ?>
	...
	<div class="form-group">
		...
		if($this->session->userdata('group_id')==1 || in_array($mainmenu['menu_function'], $this->GroupPermission)){
		?>
		...
		if($this->session->userdata('group_id')==1 || in_array($submenu['menu_function'], $this->GroupPermission)){	
			?>
        <input type="checkbox" value="<?=$submenu['menu_function'];?>" name="permission[]" /><?=$submenu['menu_title']?>
        <?php }elseif($submenu!=null && $getownmenu!=null){?>
        <label><?=$submenu['menu_title']?></label>
        <select name="permission[]" class="form-control">
        <option value="0">None</option>
        <?php if($this->session->userdata('group_id')==1 || in_array($submenu['menu_function'], $this->GroupPermission)){?>
        <option value="<?=$submenu['menu_function']?>" /><?=$submenu['menu_title']?></option>
       <?php }if($this->session->userdata('group_id')==1 || in_array($getownmenu['menu_function'], $this->GroupPermission)){?>
        <option value="<?=$getownmenu['menu_function']?>"/><?=$getownmenu['menu_title']?></option>
        <?php }?>
        </select>
        <?php }}?>
        </div>
        ...
        <input type="checkbox" value="<?=$subsubmenu['menu_function'];?>" name="permission[]" /><?=$subsubmenu['menu_title']?>
        <?php }elseif($subsubmenu!=null && $getownsubmenu!=null){?>
        <label><?=$subsubmenu['menu_title']?></label>
        <select name="permission[]" class="form-control">
        <option value="0">None</option>
        <option value="<?=$subsubmenu['menu_function']?>" /><?=$subsubmenu['menu_title']?></option>
        <option value="<?=$getownsubmenu['menu_function']?>"/><?=$getownsubmenu['menu_title']?></option>
        </select>
        <?php }?>
        </div>
        <?php
