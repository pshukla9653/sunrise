...
<?php 
//$menu_order = json_decode($this->site_setting['admin_menu_order']);
$CI =& get_instance();
$menu_order = $CI->objToArray(json_decode($this->site_setting['admin_menu_order'], true));
...
<div class="cf nestable-lists">
        ...
        <div class="dd" id="nestable">
          ...
          <?php if(count($menu_order)==0){?>  
          <ol class="dd-list">
            <?php foreach($main_menu_list as $main_menu):?>
			
                <li class="dd-item" data-id="<?php echo $main_menu['id'];?>">
                    <div class="dd-handle"><?php echo $main_menu['menu_title'];?></div>
                    <?php $submenulist1 = $this->Menu_model->get_sub_menulist($main_menu['id']);
								if($submenulist1!=''): ?>
                                <ol class="dd-list">
                                	<?php foreach($submenulist1 as $submenu1):?>
                                    	<li class="dd-item" data-id="<?php echo $submenu1['id'];?>"><div class="dd-handle"><?php echo $submenu1['menu_title'];?></div>
                                        	<?php $submenulist2 = $this->Menu_model->get_sub_menulist($submenu1['id']);
											if($submenulist2!=''): ?>
                                			<ol class="dd-list">
                                			<?php foreach($submenulist2 as $submenu2):?>
                                    		<li class="dd-item" data-id="<?php echo $submenu2['id'];?>"><div class="dd-handle"><?php echo $submenu2['menu_title'];?></div>
													<?php $submenulist3 = $this->Menu_model->get_sub_menulist($submenu2['id']);
                                                    if($submenulist3!=''): ?>
                                                    <ol class="dd-list">
                                                    <?php foreach($submenulist3 as $submenu3):?>
                                                    <li class="dd-item" data-id="<?php echo $submenu3['id'];?>"><div class="dd-handle"><?php echo $submenu3['menu_title'];?></div>
                ...
                echo    '<div class="dd-handle">'.$menu1['menu_title'].'</div>';
					...
					echo '<ol class="dd-list">';
							for($r=0; $r < count($sub_menu_order1); $r++):
								...
								echo  '</li>';
										if($menu2[$r]['children']!=null): 
										$sub_menu_order2 = $menu2[$r]['children'];
										echo '<ol class="dd-list">';
												for($j=0; $j < count($sub_menu_order2); $j++):
													$menu3 ='';
													$menu3 =$this->Menu_model->get_data(array('id'=>$sub_menu_order2[$j]['id'],'menu_parent_id'=>$menu2[$r]['id'],'display_in_menu'=>1,'status'=>1));
													echo  '<li class="dd-item" data-id="'.$menu3['id'].'">';
													echo  '<div class="dd-handle">'.$menu3['menu_title'].'</div>';
													echo  '</li>';
															if($menu3[$j]['children']!=null): 
															$sub_menu_order3 = $menu2[$j]['children'];
															echo '<ol class="dd-list">';
																	for($k=0; $k < count($sub_menu_order3); $k++):
																		$menu4 ='';
																		$menu4 =$this->Menu_model->get_data(array('id'=>$sub_menu_order3[$k]['id'],'menu_parent_id'=>$menu3[$j]['id'],'display_in_menu'=>1,'status'=>1));
