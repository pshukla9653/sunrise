 ...
 if(product_id ==''){
	...
	if (product_id !='' && service_id !=''){
		if(service_id == '1'){
			if(amc_duration !=''){
                ...
                success:function(data){
				  ...
				  html += '<td><input type="hidden" name="rowdata['+newsrno+'][amc_duration]" value="'+data.amc_duration+'"/><input type="hidden" name="rowdata['+newsrno+'][amc_end_date]" value="'+data.amc_end_date+'"/><input type="hidden" name="rowdata['+newsrno+'][amc_end_date]" value="'+data.amc_end_date+'"/><input type="hidden" name="rowdata['+newsrno+'][amc_start_date]" value="'+data.amc_start_date+'"/><input type="hidden" name="rowdata['+newsrno+'][ms_dates]" value="'+data.ms_dates+'"/><input type="hidden" name="rowdata['+newsrno+'][ms_duration]" value="'+data.ms_duration+'"/><input type="hidden" name="rowdata['+newsrno+'][product_id]" value="'+data.product_id+'"/><input type="hidden" name="rowdata['+newsrno+'][service_id]" value="'+data.service_id+'"/>'+data.product_name+'</td>';
		...
		getGrandTotal();
						...
						new PNotify({title: 'Warrning!',text: 'Invalid Error',icon: 'icon-danger',addclass: 'bg-danger'});	
					}
                 
                }
				
            }); 
				new PNotify({title: 'Success!',text: 'Product Add Successfully',icon: 'icon-success',addclass: 'bg-success'});
				}
			}
		}
			...
			if(product_id !=''){
                ...
                success:function(data){
				 ...
				 html += '<td><input type="text" name="rowdata['+newsrno+'][amount]" id="amount_'+newsrno+'" class="form-control" value="'+data.amount+'" readonly/><input type="hidden" name="amount[]" id="hidden_amount_'+newsrno+'" value="'+data.amount+'"/></td>';
				  html += '<td><input type="text" name="rowdata['+newsrno+'][discounted_amount]" id="discount_amount_'+newsrno+'" class="form-control" value="0" readonly/></td>';
				  html += '<td><span class="btn btn-info" id="removeRow"><i class="fa fa-close"</span></td>';
				  html += '</tr>';
...
function addproductpart(){
		...
		getGrandTotal();
				...
				new PNotify({title: 'Success!',text: 'Product Part Add Successfully',icon: 'icon-success',addclass: 'bg-success'});
				}
			}
			else{
				new PNotify({title: 'Warrning!',text: 'Select Product Part',icon: 'icon-danger',addclass: 'bg-danger'});
			}
}
...
function discountpercentage(rowid){
		...
		if(parseFloat(discountpercent) < 51){
			...
			new PNotify({title: 'Warrning!',text: 'Discount Percentage not more then 50%',icon: 'icon-danger',addclass: 'bg-danger'});
		}
}
...
$('#gst').on('change', function(){
	...
	if(gst_type ==''){
		new PNotify({title: 'Warrning!',text: 'Please Select GST Type!',icon: 'icon-danger',addclass: 'bg-danger'});
	}
	if(gst ==''){
		new PNotify({title: 'Warrning!',text: 'Please Select GST!',icon: 'icon-danger',addclass: 'bg-danger'});
	}
	if(gst !='' && gst_type !=''){
		if(gst =='1'){
...
function changeamcamount(rowid){
		...
		var amount = $('#amount_'+rowid).val();
		var product_id = $('#discount_amount_'+rowid).val();
