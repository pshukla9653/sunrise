...
class Leads extends Admin_Controller
{
	...
	public function gettodaymslist()
	{
		...
		if ($this->input->post('btn') == 'Search') {
			...
			$udata = $this->Leads_model->get_today_ms_list_between($datefrom, $dateto, $workassignedu);
		} else {
	...
	public function gettodayamclist()
	{
		$data['countrieslist'] 	= $this->Countries_model->get_all_countrieslist();
		$data['statelist'] 	= $this->States_model->getstatelist();
		$data['citylist'] 	= $this->City_model->getcitylist();
		...
		if ($this->input->post('btn') == 'Search') {
			...
			$udata = $this->Leads_model->get_today_expire_list_between($datefrom, $dateto, $workassignedu);
		} else {
			$udata = $this->Leads_model->get_today_expire_list(date("Y-m-d"));
		}
		$newdata =  array();
		foreach ($udata as $item) {
			//if(in_array('leads-show', $this->GroupPermission)){
		...
		$data['leadList'] = $udata;
		var_dump(array($data['leadList'][0]));
		exit;
		$this->render_template('secure/leads/amc_reminder', $data);
	}
}
