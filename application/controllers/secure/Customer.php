...
class Customer extends Admin_Controller
{
	...
	public function customerlist()
	{
		...
		foreach ($data as $item) {
			if (in_array('leads-show', $this->GroupPermission)) {
				$showbtn = '<a onClick="showdetail(' . $item['user_id'] . ')"><i class="fa fa-eye fa-2x" style="color:green; font-size:22px;"></i></a>';
			}
	...
	public function getdetail()
	{
		if ($_POST['id']) {
			$responce = array();
			$check_user = $this->Customer_model->get_data('tbl_customer', array('user_id' => $_POST['id']));
			if ($check_user['id'] == '') {
	...
	public function updateservicedetail(){
		if ($_POST['id']) {
			$responce = array();
			$check_user = $this->Customer_model->get_data('tbl_customer_product_service', array('id' => $_POST['id']));
			if ($check_user['id'] == '') {
				...
				$update_details = $this->Customer_model->get_data('tbl_customer_product_service', array('id' => $_POST['id']));
				if($update_details){
				$responce['status'] = true;
				$responce['data'] = $data;
				}
	...
	{
		$data = $this->Customer_model->get_all_list('sunrise_data', array('done' => 0));
		
		foreach ($data as $item) {
			//var_dump($item); exit;
			$data = $this->addcustomer($item);
			$this->Customer_model->update_data('sunrise_data', array('id' => $item['id']), array('done' => 1));
			echo 'Data Insert successfull with :<br>USER:<br>'.implode(',', $data['user']).'<br>CUSTOMER:<br>'.implode(',', $data['customer']).'<br>SERVICE:<br>'.implode(',', $data['service']).'<br>MSDATE:<br>'.implode(',', $data['msrecord']).'<br><br><br><br>';
		}
	}
		...
		$checkdata = $this->Customer_model->get_data('tbl_customer', array('mobile' => $item['mobile']));
			...
			if ($checkdata['id'] == '') {
				...
				if ($id) {
					...
					$cust_id = $id;
					$customer_id = $this->Customer_model->insert_data('tbl_customer', $customer);
					//$cust_id =1;
					//var_dump($customer);
					if ($customer_id) {
						$update_cu['customer_code'] = 'SAS-0' . $customer_id;
						//var_dump($update_cu); 
						$this->Customer_model->update_data('tbl_customer', array('id' => $customer_id), $update_cu);
					}
				}
			} else {
				$cust_id = $checkdata['user_id'];
			}
			$productmsdetail = $this->getproductmsdetail($item['date_from'],$item['product_id'], $item['service_id'],$item['amc_duration']);
			var_dump($item);
			var_dump($productmsdetail);
			exit;
			$service = array(
				'customer_id' => $cust_id,
				...
				'amc_start_date' => $productmsdetail['start_date'],
				'amc_end_date' => $productmsdetail['end_date'],
				'ms_no' => count($productmsdetail['msdates']),
				'ms_dates' => implode(',', $productmsdetail['msdates']),
				'address' => $item['address'],
				'discount_percent' => 0,
				'discount_amount' => 0,
				'amount' => 0,
			...
			if ($service['service_id'] == '1' || $service['service_id'] == '3') {
				...
				for ($r = 0; $r < count($msdate); $r++) {
					...
					//var_dump($msrecord);
					//var_dump('ok');
					$this->Customer_model->insert_data('tbl_customer_ms_service', $msrecord);
				}
			}
			elseif($service['service_id'] == '4'){
				$msrecord = array(
					...
					'invoice_date' => $item['date_from'],
					'address' => $service['address'],
					'ps_date' => $item['date_from'],
					'ps_end_date' => $productmsdetail['end_date'],
					'ps_status' => 0,
					'createdon' => date_timestamp_get(date_create()),
					'createdby' 		=> $this->session->userdata('id')
				);
				//var_dump($msrecord);
				//var_dump('ok');
				$this->Customer_model->insert_data('tbl_customer_ps_service', $msrecord);
			}
			$data['user']= $params;
			$data['customer'] = $customer;
			$data['service'] = $service;
			$data['msrecord'] = $msrecord;
			return $data;
	}
	public function getproductmsdetail($date, $product_id, $service_id, $amc_duration){
			$data = array();
			$msdate = array();
			$product    = $this->Customer_model->get_data('tbl_products', array('id' => $product_id));
			if ($service_id == '1' || $service_id == '3') {
				$mandtory_service = $product['mandtory_service'];
				$end = date("Y-m-d", strtotime('+'.$amc_duration.'years', strtotime($date)));
